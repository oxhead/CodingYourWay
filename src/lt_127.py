...
Related:
  - lt_126_word-ladder-ii
  - lt_433_minimum-genetic-mutation
"""
...
class Solution:
            ...
            for word in candidates:
                ...
                for i in range(len(word)):
                    for c in 'abcdefghijklmnopqrstuvwxyz':
                        ...
                        if candidate not in visited and candidate in valid_words:
    ...
    def ladderLength_bidirectional_BFS(self, beginWord, endWord, wordList):
        """
        :type beginWord: str
        :type endWord: str
        :type wordList: List[str]
        ...
        # Time: O(n * d^1/2)
        # Space: O(n)
        # https://leetcode.com/problems/word-ladder/discuss/40710/Share-my-two-Python-solutions:-a-very-concise-one-(12-lines-~160ms)-and-an-optimized-solution(~100ms)
        ...
        while front:
            ...
            front = wordDict & (set([word[:index] + ch + word[index+1:] for word in front for index in range(len(beginWord)) for ch in 'abcdefghijklmnopqrstuvwxyz']))
            if front & back:
...
if __name__ == '__main__':
    test_cases = [
        ...
        (("sand", "acne", ["slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks", "gang", "pool", "mock", "fort", "heel", "send",
...
"torn", "cups", "worm", "baum", "axon", "purr", "vise", "grew", "govs", "meat", "chef", "rest", "lame"]), 11),
    ]

    for test_case in test_cases:
        print('case:', test_case)
        output = Solution().ladderLength(*test_case[0])
        print('output:', output)
